...
// Carrito de Compras +  Array con objetos ProductoEnCarrito agregados al carrito
class Carrito {
    constructor(listaDeProductosEnCarrito) {
        this.listaDeProductosEnCarrito = listaDeProductosEnCarrito; 
    }
    ...
    // Agrega un producto al carrito de compras, Si ya existe el producto en el carrito, actualizo la cantidad
    ...
    // Devuelvo true si el producto especificado se encuentra en el carrito
    ...
    // Obtengo el producto especificado del carrito de compras
    ...
    // Actualizamos la cantidad del producto en el carrito de compras
...
// Busca los productos al hacer click al boton Buscar y Cancelo la accion por defecto del evento
function buscarProducto(event) {
    ...
    // Busco los productos que coinicidan con el valor del texto en el cuadro de busqueda y renderizamos el array resultado
...
function limpiarBusqueda(event) {
    ...
    if (event.target.value === "") {
        // Si el valor esta vacio, se clickeo en la x del input search para vaciar la busqueda,entonces vuelvo a mostrar todos los productos
...
function renderProducto(oFila, oProducto) {
    ...
    oLabel.setAttribute("for", "cantidad" + id);  
    ...
    oCantidad.id = "cantidad" + id; 
    oCantidad.name = oCantidad.id; 
    ...
    oAddCart.classList = "my__button my__button__text";
    ...
    oBuyButton.classList = "page-wrapper";
    oBuyButton.innerText = "🛒";
...
function comprar(event) {
    ...
    // Limpiamo el carrito antes de la compra directa
    vaciarCarrito();

    // Agregamos el producto seleccionado para la compra directa
    agregarProductoAlCarritoHTML(oHTMLBotonAgregar);

    // Redirecciamos a la pagina de compra
...
function agregarProductoAlCarritoHTML(oHTMLBotonAgregar) {
    ...
    if (oProductoSeleccionado !== null) {
        ...
        if (oProductoSeleccionado.hayStock(cantidadAComprar)) {
            ...
            renderToast(`Se agregó ${cantidadAComprar} ${oProductoSeleccionado.descripcion} al carrito`);
...
function agregarEventosBtnComprar() {
    ...
    const btnsAgregarCarrito = document.getElementsByClassName("my__button my__button__text");
    ...
    const btnsComprar = document.getElementsByClassName("page-wrapper");
